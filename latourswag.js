...
function make_dhsi18() {

  // set up our two data sources
  // URI for all recent #swag tweets with 'and' in them
  var swag = {q: "#dhsi18 and", count: 100, result_type: "recent"};  
  // URI for all recent @latourbot tweets
  var latour = {q: "from:TwitterAPI", count: 100, result_type: "recent"}; 
  var swags = [];
  var latours = [];
  var latoursShort = [];
  var pre = "";
  ...
  // get the swag tweets and parse into a javascript object called 'data'
    T.get('search/tweets', swag, function(err, data) {   
    if (err) throw err;
    // jettison the metadata, we just care about the status update part of the search
